...
## macOS PORT — SPEC APPROVED 2026-07-07, IN EXECUTION (MACOS_PORT_SPEC.md)
Was: future project (Joe-requested 2026-07-07 at installer approval). Now
specced via the full ROUTING pipeline (drafter -> planner -> Joe approval +
Amendment A1) — MACOS_PORT_SPEC.md is the record. Phase 1 (Windows-side:
fail-closed ensureOrtLoaded + mac dlopen branch, Menlo font select, inert
CMake mac branch) executes now; Phase 2 (CI build on GitHub Actions +
remote-friend Zoom validation) is gated on Joe's private-GitHub push.
Ship-gate items (Apple Developer enrollment for distribution signing/
notarization; JUCE purchase) remain in "BEFORE PUBLIC RELEASE" below.
