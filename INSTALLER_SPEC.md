...
| T6 | (this commit) | README.md gained an "Install (no build)" section above the existing build-from-source path (kept fully intact), covering `installer\make_installer.bat` -> `dist\GentSamplerSetup-<ver>.exe` -> double-click install to the standard VST3 folder (any Windows VST3 DAW) -> uninstall via Add/Remove, with the private-distribution warning stated plainly (JUCE commercial purchase pending, see THIRD_PARTY_LICENSES); PACKAGING_SPEC.md got exactly the one specified pointer sentence appended to its Joe close-out paragraph, nothing else touched; BACKLOG.md's INSTALLER item marked RESOLVED 2026-07-07 citing T1-T5 + Joe's clean-machine PASS, replaced with a surviving "BEFORE PUBLIC RELEASE" item listing the three gates (JUCE purchase / code-signing / EULA decision), macOS PORT item untouched; CLAUDE.md left untouched per this dispatch's explicit scope restriction (README.md/PACKAGING_SPEC.md/BACKLOG.md/INSTALLER_SPEC.md only — CLAUDE.md update deferred to wave/spec close). Gates: build_test.sh green (Release build + ctest), pluginval_gate.sh SUCCESS, no cold-open timeout (text-only change, gates confirm no accidental damage). | pending | |
